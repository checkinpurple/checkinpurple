...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      // Safety net: never stay on spinner forever
  ...
  const fetchUserProfile = async (userId: string) => {
    try {
      ...
      // If no profile row yet, create one from auth metadata
      if (!profile && authUser) {
      ...
      const fallbackProfiles = Array.isArray(metadata?.profiles)
        ? metadata.profiles
      ...
      if (profile) {
        ...
        // No profile row yet — use auth metadata so UI doesn't stay blank
        setUser({
          id: authUser.id,
          email: authUser.email || '',
      ...
      // Don't leave user in limbo — clear and let them re-login
      setUser(null);
    } finally {
      // Always resolve loading regardless of which branch we took
      setLoading(false);
    }
  };
