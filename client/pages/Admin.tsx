...
export default function Admin() {
  ...
  const loadUsers = async () => {
    try {
      ...
      setUsers(userList);
      // Use total from auth (true registration count) if available
      const total = d?.total ?? userList.length;
                              ...
                              <td className="px-5 py-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    {u.has_profile === false && (
                                      <span className="ml-1.5 text-xs px-1.5 py-0.5 rounded bg-yellow-500/10 text-yellow-500 border border-yellow-500/20">No Profile</span>
                                    )}
                                  </div>
                                </div>
                              </td>
