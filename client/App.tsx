...
function ProtectedRoute({
  ...
  // Check both current role AND all profiles the user has unlocked
  const userProfiles: string[] = user.profiles?.length
    ? user.profiles
    : [user.role];

  if (requiredRole) {
    const hasRole =
      userProfiles.includes(requiredRole) ||
      user.role === requiredRole ||
      // artist_fan legacy role
      (requiredRole === "artist" && (user.role === "artist_fan" || userProfiles.includes("artist"))) ||
      (requiredRole === "fan" && (user.role === "artist_fan" || userProfiles.includes("fan")));
