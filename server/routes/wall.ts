  ...
  try {
    ...
    const cataloguePosts = Array.from(productsByMerchant.entries()).slice(0, 8).map(([merchantId, products]) => {
      ...
      return {
        ...
        timestamp: "Recently",
        caption: "Fresh store drops now live in the CheckinPurple store.",
    ...
    const reelPosts = users
      ...
      .map((u, idx) => ({
        ...
        timestamp: "Recently",
        caption: "Quick studio reel — new music in progress.",
        thumbnailUrl: `https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=600&sig=${idx}`,
        duration: "0:30",
