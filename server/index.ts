...
import { setMerchantAvailability, setDealStatus } from "./routes/merchant-influencer-extras";
...
export function createServer() {
  ...
  // Public stats (real user count from auth)
  app.get("/api/public/stats", async (_req, res) => {
    try {
      // auth.admin.listUsers gives the true registration count
      const { data, error } = await supabase.auth.admin.listUsers({ perPage: 1 });
      if (error) throw error;
      // total_count is in the response when perPage=1
      const total = (data as any)?.total ?? data?.users?.length ?? 0;
      // Fallback: count profile rows if auth count unavailable
      if (!total) {
  ...
  // Merchant availability + influencer deal status (auto-wall-posts)
  app.post("/api/merchant/availability", setMerchantAvailability);
  app.patch("/api/influencer/deals/:id", setDealStatus);
