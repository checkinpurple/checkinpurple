...
CREATE TABLE users (
  ...
  role VARCHAR CHECK (role IN ('artist', 'fan', 'merchant', 'influencer', 'artist_fan', 'admin')) NOT NULL,
  avatar_url VARCHAR,
  bio TEXT,
  location TEXT,
  website TEXT,
...
CREATE POLICY "Artists can manage own artist profile" ON artist_profiles
...
CREATE POLICY "Anyone can view artist events" ON artist_events
  FOR SELECT USING (true);
...
CREATE TABLE artist_booking_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE POLICY "Users can view own booking requests" ON artist_booking_requests
  FOR SELECT USING (auth.uid() = fan_id OR auth.uid() = artist_id);

CREATE POLICY "Fans can create booking requests" ON artist_booking_requests
  FOR INSERT WITH CHECK (auth.uid() = fan_id);

CREATE POLICY "Artists can update booking requests" ON artist_booking_requests
...
INSERT INTO subscription_tiers (name, description, price_monthly, track_limit, profile_limit, features) VALUES
('Basic', 'Create and stream live audio content - Limited to 10 tracks', 0.00, 10, 1, '{"live_streaming": true, "basic_analytics": true, "track_limit": 10}'),
('Standard', 'Everything in Basic plus scheduled releases and bookings - Unlimited tracks', 19.99, NULL, 2, '{"live_streaming": true, "scheduled_releases": true, "bookings": true, "advanced_analytics": true, "unlimited_tracks": true}'),
('Premium', 'All profiles unlocked plus everything in Standard', 49.99, NULL, 4, '{"live_streaming": true, "scheduled_releases": true, "bookings": true, "advanced_analytics": true, "unlimited_tracks": true, "all_profiles": true}');
